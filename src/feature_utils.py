...
def extract_features():
    ...
    stk_tickers = ['AMD', 'NVDA', 'WM', 'TSLA', 'TXN']
    ccy_tickers = ['DEXJPUS', 'DEXUSEU']
    ...
    Y = np.log(stk_data.loc[:, ('Adj Close', 'AMD')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1]+'_Future'
    
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('NVDA', 'WM', 'TSLA', 'TXN'))]).diff(return_period)
